// kredi/app/admin/kredi-kartlari/ticari-kartlari/page.tsx
"use client";
...
const ManageCommercialCreditCardsPage = () => {
  ...
  const fetchCards = async () => {
    try {
      ...
      const data = await response.json();
      setCards(data.filter((card: CreditCard) => card.card_type === 'commercial_card'));
    } catch (err: any) {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-4">{editingCard ? 'Ticari Kredi Kartını Düzenle' : 'Ticari Kredi Kartı Ekle ve Listele'}</h1>
      ...
      <div>
        <h2 className="text-xl font-semibold mb-3">Mevcut Ticari Kredi Kartları</h2>
...
export default ManageCommercialCreditCardsPage;
