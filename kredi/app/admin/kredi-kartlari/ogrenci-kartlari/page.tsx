// kredi/app/admin/kredi-kartlari/ogrenci-kartlari/page.tsx
"use client";
...
const ManageStudentCreditCardsPage = () => {
  ...
  const fetchCards = async () => {
    try {
      ...
      const data = await response.json();
      setCards(data.filter((card: CreditCard) => card.card_type === 'student_card'));
    } catch (err: any) {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-4">{editingCard ? 'Öğrenci Kredi Kartını Düzenle' : 'Öğrenci Kredi Kartı Ekle ve Listele'}</h1>
      ...
      <div>
        <h2 className="text-xl font-semibold mb-3">Mevcut Öğrenci Kredi Kartları</h2>
...
export default ManageStudentCreditCardsPage;
