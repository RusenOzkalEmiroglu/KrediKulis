// kredi/app/admin/kredi-kartlari/aidatsiz-kartlar/page.tsx
"use client";
...
const ManageFeeFreeCreditCardsPage = () => {
  ...
  const fetchCards = async () => {
    try {
      ...
      const data = await response.json();
      setCards(data.filter((card: CreditCard) => card.card_type === 'fee_free'));
    } catch (err: any) {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-4">{editingCard ? 'Aidatsız Kredi Kartını Düzenle' : 'Aidatsız Kredi Kartı Ekle ve Listele'}</h1>
      ...
      <div>
        <h2 className="text-xl font-semibold mb-3">Mevcut Aidatsız Kredi Kartları</h2>
...
export default ManageFeeFreeCreditCardsPage;
