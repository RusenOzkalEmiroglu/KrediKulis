...
import { Car, CheckCircle, Clock, Calendar, ArrowRight, Loader2 } from "lucide-react";
...
export default function TasitKredisi() {
  ...
  const updateDisplayLoans = (baseAmount: number, baseTerm: number, products: LoanProduct[]) => {
    const updatedLoans = products.map(product => {
      ...
      const maks_vade = product.maks_vade || 48;
  ...
  useEffect(() => {
    ...
    const fetchAllLoans = async () => {
      console.log("Fetching initial loan data for vehicle page...");
      setIsLoading(true);
      try {
        // Set up both fetches to run in parallel
        const vehicleLoansPromise = supabase
          .from('vehicle_loans')
        ...
        const [vehicleLoansResponse, interestFreeResponse] = await Promise.all([
          vehicleLoansPromise,
          interestFreePromise
        ]);

        // Handle vehicle loans
        if (vehicleLoansResponse.error) throw vehicleLoansResponse.error;
        if (vehicleLoansResponse.data) {
          console.log("Fetched vehicle loans:", vehicleLoansResponse.data);
          const shuffledData = shuffle(vehicleLoansResponse.data);
  ...
  const handleCardChange = (id: number, field: 'amount' | 'term', value: string) => {
    const newDisplayLoans = displayLoans.map(loan => {
      if (loan.id === id) {
        ...
        const maks_vade = loan.maks_vade || 48;
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="bg-white p-6 rounded-lg shadow-md mb-10">
        <div className="bg-[#ff3d00] text-white rounded-lg p-4 mb-4">
          <h2 className="text-xl font-semibold">Taşıt Kredisi Hesaplama</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
          ...
          <div>
            ...
            >
              ...
              <option value="2. el">İkinci El</option>
            </select>
          </div>
          <button 
        ...
        {/* Banka Teklifleri */}
        <div>
          {displayLoans.filter(loan => loan.loan_type === '0').length > 0 && (
            <div>
              <h3 className="text-2xl font-semibold mb-4">Sıfır Araç Kredileri</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mt-8">
                {displayLoans.filter(loan => loan.loan_type === '0').map(loan => (
                  <div 
                    key={loan.id} 
          ...
          {displayLoans.filter(loan => loan.loan_type === '2. el').length > 0 && (
            <div className="mt-10">
              <h3 className="text-2xl font-semibold mb-4">İkinci El Araç Kredileri</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mt-8">
                {displayLoans.filter(loan => loan.loan_type === '2. el').map(loan => (
                  <div 
                    key={loan.id} 
                    className="bg-white rounded-xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 border-t-4 hover:scale-[1.02] cursor-pointer"
                    style={{ borderColor: loan.bank?.color || '#808080' }}
          ...
          {!isLoading && displayLoans.length === 0 && <p>Gösterilecek kredi ürünü bulunamadı.</p>}
        </div>
      </div>
