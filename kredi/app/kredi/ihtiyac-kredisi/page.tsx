...
import { ShoppingBag, CheckCircle, DollarSign, Clock, Calendar, ArrowRight, Loader2 } from "lucide-react";
...
interface LoanProduct {
  ...
  maks_tutar: number;
  maks_vade: number;
  bank: Bank; // Corrected from 'banks'
  loan_type: string;
  offer_link: string;
...
export default function IhtiyacKredisi() {
  ...
  const [interestFreeLoans, setInterestFreeLoans] = useState<LoanProduct[]>([]);
  ...
  const updateDisplayLoans = (baseAmount: number, baseTerm: number, products: LoanProduct[]) => {
    const updatedLoans = products.map(product => {
      ...
      const maks_vade = product.maks_vade || 36;
  ...
  useEffect(() => {
    ...
    const fetchLoans = async () => {
      console.log("Fetching initial loan data...");
      setIsLoading(true);
      try {
        // Fetch consumer loans and interest-free loans in parallel
        const [consumerLoansResponse, interestFreeLoansResponse] = await Promise.all([
          fetch('/api/consumer-loans', { cache: 'no-store' }),
          fetch('/api/zero-interest-loans', { cache: 'no-store' })
        ]);

        if (!consumerLoansResponse.ok) {
          throw new Error('Failed to fetch consumer loans');
        }
        if (!interestFreeLoansResponse.ok) {
          throw new Error('Failed to fetch interest-free loans');
        }

        const consumerLoansData = await consumerLoansResponse.json();
        const interestFreeLoansData = await interestFreeLoansResponse.json();

        if (consumerLoansData) {
          const shuffledData = shuffle(consumerLoansData as LoanProduct[]);
          setLoanProducts(shuffledData);
          updateDisplayLoans(parseFloat(amount), parseInt(term, 10), shuffledData);
        }

        if (interestFreeLoansData) {
          console.log("Fetched interestFree loans:", interestFreeLoansData);
          setInterestFreeLoans(interestFreeLoansData as LoanProduct[]);
        }
  ...
  const handleCardChange = (id: number, field: 'amount' | 'term', value: string) => {
    const newDisplayLoans = displayLoans.map(loan => {
      if (loan.id === id) {
        ...
        const maks_vade = loan.maks_vade || 36;
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="bg-white p-6 rounded-lg shadow-md mb-10">
        <div className="bg-[#ff3d00] text-white rounded-lg p-4 mb-4">
          <h2 className="text-xl font-semibold">İhtiyaç Kredisi Hesaplama</h2>
        </div>
